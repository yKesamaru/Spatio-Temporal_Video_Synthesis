...
from damo_yolo.damo_internal.config import Config as MyConfig
...
class Config(MyConfig):
    def __init__(self):
        ...
        # backbone設定
        # self.read_structureで使用するパスを絶対パスに修正
        current_dir = os.path.dirname(os.path.abspath(__file__))  # 現在のファイルのディレクトリ
        structure_path = os.path.join(
            current_dir, "../damo_internal/base_models/backbones/nas_backbones/tinynas_L20_k1kx.txt"
        )

        # パスを使用して構造を読み込む
        structure = self.read_structure(structure_path)
